...
const GroupRoomScreen = () => {
  ...
  // Check group membership on mount
  useEffect(() => {
    async function checkMembership() {
      ...
      try {
        ...
        setIsMember(true); // fallback to allow
      }
    }
    checkMembership();
  }, [user?.id, groupId]);

  // Fix handleSendMessage: use user.id, check membership, loading state, only clear input on success
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={{
        ...
        {/* If not a member, show error message instead of input */}
        {!isMember && (
          <View style={{ padding: 16, backgroundColor: COLORS.inputBg, alignItems: 'center' }}>
